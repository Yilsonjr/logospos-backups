...
\restrict 81Vy9ufiwHMB7HEEuPnadekCltvp5a2gXwRQmtI9FUJHoed4aYAw1NsIbuuaqdS
...
CREATE FUNCTION public.calcular_limite_credito(p_proveedor_id uuid) RETURNS numeric
    ...
    AS $$
declare
  v_finca     finca_proveedor%rowtype;
  v_scoring   categoria_scoring;
  v_factor    numeric;
  v_limite    numeric;
begin
  select * into v_finca 
  from finca_proveedor 
  where proveedor_id = p_proveedor_id;
  
  if not found then return 0; end if;
  
  -- Obtener scoring si existe
  select categoria into v_scoring
  from scoring_proveedor
  where proveedor_id = p_proveedor_id;
  
  -- Factor según scoring
  v_factor := case v_scoring
    when 'A' then 1.00
    when 'B' then 0.75
    when 'C' then 0.50
    else v_finca.factor_riesgo  -- nuevo cliente: usa el factor manual
  end;
  
  -- Cálculo base
  v_limite := coalesce(v_finca.quintales_estimados_año, 0) 
    * v_finca.precio_referencia_qq 
    * v_factor;
  
  -- Actualizar el campo sugerido
  update finca_proveedor
  set limite_credito_sugerido = round(v_limite, 2)
  where proveedor_id = p_proveedor_id;
...
--
-- Name: registrar_abono(uuid, numeric, text, uuid); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.registrar_abono(p_compra_id uuid, p_monto numeric, p_metodo text, p_registrado_por uuid) RETURNS void
    LANGUAGE plpgsql
    AS $$
declare
  v_total        numeric;
...
begin
  ...
  if p_monto <= 0 then raise exception 'Monto debe ser mayor a cero'; end if;

  v_pagado_nuevo := v_pagado + p_monto;
  v_saldo_nuevo  := v_total - v_pagado_nuevo;

  if v_saldo_nuevo < 0 then raise exception 'Abono supera el saldo pendiente'; end if;
  ...
  insert into abono_proveedor (compra_id, proveedor_id, monto, metodo_pago, registrado_por)
  select p_compra_id, proveedor_id, p_monto, p_metodo::metodo_pago, p_registrado_por
  from compra where id = p_compra_id;
...
ALTER FUNCTION public.registrar_abono(p_compra_id uuid, p_monto numeric, p_metodo text, p_registrado_por uuid) OWNER TO postgres;

--
-- Name: registrar_pago_prestamo(uuid, numeric, text, uuid, text, text); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.registrar_pago_prestamo(p_prestamo_id uuid, p_monto numeric, p_metodo text, p_registrado_por uuid, p_nota text DEFAULT NULL::text, p_referencia text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql
    SET search_path TO 'public'
    AS $_$
declare
  ...
  v_restante    numeric;
  v_a_mora      numeric := 0;
  v_a_interes   numeric := 0;
  v_a_capital   numeric := 0;
  v_interes_hoy numeric;
  v_deuda       numeric;
  v_cuota       cuota_prestamo%rowtype;
  v_cuota_id    uuid := null;
  v_tipo        tipo_pago_prestamo;
  v_estado      estado_prestamo;
  v_saldo_nuevo numeric;
  v_mora_nueva  numeric;
begin
  ...
  if v_prestamo.estado in ('pagado', 'cancelado') then
    raise exception 'Este préstamo ya está %', v_prestamo.estado;
  end if;
  ...
  v_restante    := p_monto;
  v_interes_hoy := calcular_interes_prestamo(p_prestamo_id, hoy_rd());

  -- FIX 1: rechazar el excedente en vez de tragárselo.
  v_deuda := v_prestamo.mora_acumulada
           + (case when v_prestamo.tasa_interes_pct > 0 then coalesce(v_interes_hoy, 0) else 0 end)
           + v_prestamo.saldo_capital;

  if p_monto > v_deuda + 0.005 then
    raise exception 'El pago excede la deuda total. Deuda hoy: RD$ %. Recibido: RD$ %.',
      to_char(v_deuda, 'FM999999990.00'), to_char(p_monto, 'FM999999990.00');
  end if;

  -- Imputación mora → interés → capital (igual que la versión original).
  if v_prestamo.tasa_interes_pct > 0 then
    if v_prestamo.mora_acumulada > 0 then
      v_a_mora   := least(v_restante, v_prestamo.mora_acumulada);
      v_restante := v_restante - v_a_mora;
    end if;
    if v_restante > 0 and v_interes_hoy > 0 then
      v_a_interes := least(v_restante, v_interes_hoy);
      v_restante  := v_restante - v_a_interes;
    end if;
  end if;

  if v_restante > 0 then
    v_a_capital := least(v_restante, v_prestamo.saldo_capital);
  end if;

  v_saldo_nuevo := v_prestamo.saldo_capital  - v_a_capital;
  v_mora_nueva  := v_prestamo.mora_acumulada - v_a_mora;

  -- FIX 3: marcar la cuota más vieja pendiente si el pago la cubre completa.
  if v_prestamo.modalidad_pago = 'cuotas' then
    select * into v_cuota
      from cuota_prestamo
    ...
    if found and p_monto >= v_cuota.monto_total then
      update cuota_prestamo
         set pagado = true, fecha_pago = hoy_rd()
       where id = v_cuota.id;
      v_cuota_id := v_cuota.id;
    end if;
  end if;

  -- FIX 4: el estado refleja la situación real, no solo el caso "pagado".
  v_estado := case
                when v_saldo_nuevo <= 0 then 'pagado'::estado_prestamo
                when v_mora_nueva  > 0  then 'en_mora'::estado_prestamo
                else 'al_dia'::estado_prestamo
              end;

  v_tipo := case
              when v_estado = 'pagado'                     then 'cancelacion'::tipo_pago_prestamo
              when v_prestamo.modalidad_pago = 'cuotas'    then 'cuota'::tipo_pago_prestamo
              else 'abono'::tipo_pago_prestamo
            end;

  insert into pago_prestamo (
    prestamo_id, cuota_id, registrado_por, monto_total,
    aplicado_mora, aplicado_interes, aplicado_capital,
    tipo, metodo_pago, referencia, nota, fecha
  ) values (
    p_prestamo_id, v_cuota_id, p_registrado_por, p_monto,
    v_a_mora, v_a_interes, v_a_capital,
    v_tipo, p_metodo::metodo_pago, p_referencia, p_nota,
    now()   -- FIX 2: instante real, no medianoche
  );

  update prestamo set
    saldo_capital     = v_saldo_nuevo,
    mora_acumulada    = v_mora_nueva,
    interes_acumulado = greatest(0, coalesce(v_interes_hoy, 0) - v_a_interes),
    total_pagado      = total_pagado + p_monto,
    estado            = v_estado
  where id = p_prestamo_id;

  perform recalcular_scoring(v_prestamo.proveedor_id);

  return jsonb_build_object(
    'aplicado_mora',    v_a_mora,
    'aplicado_interes', v_a_interes,
    'aplicado_capital', v_a_capital,
...
ALTER FUNCTION public.registrar_pago_prestamo(p_prestamo_id uuid, p_monto numeric, p_metodo text, p_registrado_por uuid, p_nota text, p_referencia text) OWNER TO postgres;
...
797d9d10-169c-49f0-bf08-c3460413c056	2026-08-12 15:32:07.166696-04	2026-08-12 15:32:07.166696-04	password	3ad45d33-dc77-4a4e-84c5-f81233d17883
44dbae89-7aa3-4fc2-b071-96bcbcfeb0f2	2026-08-13 10:31:34.71452-04	2026-08-13 10:31:34.71452-04	password	f88acc11-6d31-4899-898c-7af82f4093bc
...
00000000-0000-0000-0000-000000000000	154	hqfmpkjfomlp	50471f55-7b80-41ea-86ee-4e91338d3864	f	2026-08-12 15:32:07.154822-04	2026-08-12 15:32:07.154822-04	\N	797d9d10-169c-49f0-bf08-c3460413c056
...
00000000-0000-0000-0000-000000000000	156	auhars3sqcdx	50471f55-7b80-41ea-86ee-4e91338d3864	t	2026-08-13 10:31:34.676086-04	2026-08-13 17:55:01.410907-04	\N	44dbae89-7aa3-4fc2-b071-96bcbcfeb0f2
00000000-0000-0000-0000-000000000000	159	nlftktz5pnsb	50471f55-7b80-41ea-86ee-4e91338d3864	t	2026-08-13 17:55:01.420408-04	2026-08-13 18:53:09.294248-04	auhars3sqcdx	44dbae89-7aa3-4fc2-b071-96bcbcfeb0f2
...
00000000-0000-0000-0000-000000000000	161	vlf3gp6w2dvn	94610e54-aa86-4ed9-9ba5-c82d0fc079cb	f	2026-08-13 18:55:19.591144-04	2026-08-13 18:55:19.591144-04	ro4vldfyxgvf	c9fc70f5-c7d6-4c5c-be40-279164466307
00000000-0000-0000-0000-000000000000	160	fkq2mkmfivkr	50471f55-7b80-41ea-86ee-4e91338d3864	t	2026-08-13 18:53:09.307447-04	2026-08-13 21:05:33.151747-04	nlftktz5pnsb	44dbae89-7aa3-4fc2-b071-96bcbcfeb0f2
00000000-0000-0000-0000-000000000000	162	2bc5ylv53pkf	50471f55-7b80-41ea-86ee-4e91338d3864	f	2026-08-13 21:05:33.161787-04	2026-08-13 21:05:33.161787-04	fkq2mkmfivkr	44dbae89-7aa3-4fc2-b071-96bcbcfeb0f2
00000000-0000-0000-0000-000000000000	163	fs4v7jnlkbgb	eaf76eb2-446d-4f5b-97d1-fefea5fd2ef7	f	2026-08-13 22:11:12.492034-04	2026-08-13 22:11:12.492034-04	\N	1c4ea36e-2852-454b-bd60-44daa7e2880e
\.
...
c9fc70f5-c7d6-4c5c-be40-279164466307	94610e54-aa86-4ed9-9ba5-c82d0fc079cb	2026-08-11 23:27:16.781414-04	2026-08-13 18:55:19.603601-04	\N	aal1	\N	2026-08-13 22:55:19.603501	Mozilla/5.0 (iPhone; CPU iPhone OS 18_7 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 Version/3.9.3  Bluefy/3.9.3	38.9.19.25	\N	\N	\N	\N	\N
44dbae89-7aa3-4fc2-b071-96bcbcfeb0f2	50471f55-7b80-41ea-86ee-4e91338d3864	2026-08-13 10:31:34.641082-04	2026-08-13 21:05:33.177391-04	\N	aal1	\N	2026-08-14 01:05:33.177289	Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/151.0.0.0 Safari/537.36 Edg/151.0.0.0	38.9.19.25	\N	\N	\N	\N	\N
1c4ea36e-2852-454b-bd60-44daa7e2880e	eaf76eb2-446d-4f5b-97d1-fefea5fd2ef7	2026-08-13 22:11:12.471878-04	2026-08-13 22:11:12.471878-04	\N	aal1	\N	\N	Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/151.0.0.0 Safari/537.36	38.188.21.51	\N	\N	\N	\N	\N
797d9d10-169c-49f0-bf08-c3460413c056	50471f55-7b80-41ea-86ee-4e91338d3864	2026-08-12 15:32:07.138588-04	2026-08-12 15:32:07.138588-04	\N	aal1	\N	\N	Mozilla/5.0 (iPhone; CPU iPhone OS 18_7 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 Version/3.9.3  Bluefy/3.9.3	200.88.239.185	\N	\N	\N	\N	\N
\.
...
00000000-0000-0000-0000-000000000000	94610e54-aa86-4ed9-9ba5-c82d0fc079cb	authenticated	authenticated	misael14@gmail.com	$2a$10$weStFflgXOp81yaxIo8ni.l8UypkZTcKXg4PcrItRcSoO.hY.QP46	2026-08-11 23:25:45.870644-04	\N		\N		\N			\N	2026-08-11 23:27:16.780197-04	{"provider": "email", "providers": ["email"]}	{"email_verified": true}	\N	2026-08-11 23:25:45.847603-04	2026-08-13 18:55:19.593281-04	\N	\N			\N		0	\N		\N	f	\N	f
00000000-0000-0000-0000-000000000000	50471f55-7b80-41ea-86ee-4e91338d3864	authenticated	authenticated	admin@cacao.com	$2a$10$QqVsNIR5pPGBkINSDraeQusBOT7CR1wfwIesIJlt2VD0qVKYFG4Vy	2026-07-04 15:51:10.955031-04	\N		\N		\N			\N	2026-08-13 10:31:34.640986-04	{"provider": "email", "providers": ["email"]}	{"email_verified": true}	\N	2026-07-04 15:51:10.946365-04	2026-08-13 21:05:33.167308-04	\N	\N			\N		0	\N		\N	f	\N	f
00000000-0000-0000-0000-000000000000	eaf76eb2-446d-4f5b-97d1-fefea5fd2ef7	authenticated	authenticated	adriel@gmail.com	$2a$10$XZ6x8AeS1JDDuOEtWg8HIOdG75xXjpa2spmQyjh5/ozSX3hkTfYsC	2026-07-08 19:30:19.666443-04	\N		\N		\N			\N	2026-08-13 22:11:12.470787-04	{"provider": "email", "providers": ["email"]}	{"email_verified": true}	\N	2026-07-08 19:30:19.633925-04	2026-08-13 22:11:12.508244-04	\N	\N			\N		0	\N		\N	f	\N	f
\.
...
a1000000-0000-0000-0000-000000000003	Saco pequeño	Saco de menor capacidad	t	2026-07-31 14:40:24.542155-04
a1000000-0000-0000-0000-000000000004	Saco grande	Saco de mayor capacidad	t	2026-07-31 14:40:24.542155-04
a1000000-0000-0000-0000-000000000005	Saco yute	Saco de yute tradicional	t	2026-07-31 14:40:24.542155-04
\.
...
2cabd5a2-ddd5-4d58-b907-02facc09e6c2	a1000000-0000-0000-0000-000000000003	baba	\N	2026-07-31 14:40:24.542155-04
39abab74-d8bf-4dba-a6f8-442493348c63	a1000000-0000-0000-0000-000000000004	seco	\N	2026-07-31 14:40:24.542155-04
d968ad22-4d03-4794-9b83-9182e9a15a6a	a1000000-0000-0000-0000-000000000004	baba	\N	2026-07-31 14:40:24.542155-04
93267baf-7cd3-4230-898a-9cf87658b7bd	a1000000-0000-0000-0000-000000000005	seco	\N	2026-07-31 14:40:24.542155-04
486e1de4-d8ef-4de2-87d5-1231653dd407	a1000000-0000-0000-0000-000000000005	baba	\N	2026-07-31 14:40:24.542155-04
\.
...
COPY public.finca_proveedor (id, proveedor_id, tareas_sembradas, quintales_estimados_anio, variedad_cacao, ubicacion_finca, created_at, limite_credito_sugerido, limite_credito_aprobado, factor_riesgo, precio_referencia_qq) FROM stdin;
\.
...
COPY public.fondo_diario (id, vendedor_id, fecha, monto_asignado, monto_pagado, diferencia, cerrado, jornada_id) FROM stdin;
18c74f8f-a45d-49f3-b531-4f031d06e501	2530a2db-1e70-4dfc-8d61-c5310112b4ff	2026-08-02	2000000.00	0.00	\N	f	628ad2d2-0634-4996-9d42-fb8b383ecb3b
d1910ef2-cc50-4ada-9710-5c6b24c63092	2530a2db-1e70-4dfc-8d61-c5310112b4ff	2026-08-11	50000.00	0.00	\N	f	f600dca4-6139-4471-aaf1-c16035f4ce92
b2bbb443-b68c-4440-8e3f-39688425d7c2	89db70a1-e8ec-408f-a87a-906a65a9a990	2026-08-13	500000.00	0.00	\N	f	\N
\.
...
COPY public.jornada (id, numero, fecha_apertura, abierta_en, cerrada_en, estado, abierta_por, cerrada_por, precio_apertura_id, sintetica, nota, compras_count, kg_seco, kg_baba, rd_seco, rd_baba, rd_total, pagado_efectivo, deuda_generada, abonos_cobrados, detalle_vendedores, cuadre_calculado_en, recalculado_en, recalculado_por, created_at) FROM stdin;
3b961f9d-0858-496d-90b2-4fd38572ecff	1	2026-07-30	2026-07-30 16:55:27-04	2026-07-31 19:50:07-04	cerrada_sin_cuadre	50471f55-7b80-41ea-86ee-4e91338d3864	\N	\N	t	Reconstruida por la migración 016	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	2026-08-12 19:24:44.394187-04
1bd063c3-1b79-496f-bd4e-71679868f831	2	2026-07-31	2026-07-31 19:50:07-04	2026-08-02 14:57:00-04	cerrada_sin_cuadre	50471f55-7b80-41ea-86ee-4e91338d3864	\N	\N	t	Reconstruida por la migración 016	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	2026-08-12 19:24:44.394187-04
628ad2d2-0634-4996-9d42-fb8b383ecb3b	3	2026-08-02	2026-08-02 14:57:00-04	2026-08-08 16:06:16-04	cerrada_sin_cuadre	50471f55-7b80-41ea-86ee-4e91338d3864	\N	\N	t	Reconstruida por la migración 016	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	2026-08-12 19:24:44.394187-04
e3ed5169-28c3-4635-8309-cda8a20ffdde	4	2026-08-08	2026-08-08 16:06:16-04	2026-08-10 08:57:44-04	cerrada_sin_cuadre	50471f55-7b80-41ea-86ee-4e91338d3864	\N	\N	t	Reconstruida por la migración 016	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	2026-08-12 19:24:44.394187-04
b9377f71-12fd-4143-9227-9c897c40affe	5	2026-08-10	2026-08-10 08:57:44-04	2026-08-11 16:31:43-04	cerrada_sin_cuadre	50471f55-7b80-41ea-86ee-4e91338d3864	\N	\N	t	Reconstruida por la migración 016	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	2026-08-12 19:24:44.394187-04
f600dca4-6139-4471-aaf1-c16035f4ce92	6	2026-08-11	2026-08-11 16:31:43-04	2026-08-12 15:32:19-04	cerrada_sin_cuadre	50471f55-7b80-41ea-86ee-4e91338d3864	\N	\N	t	Reconstruida por la migración 016	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	2026-08-12 19:24:44.394187-04
0e641f66-5e9d-4337-a987-92e4bdb20f61	7	2026-08-12	2026-08-12 15:32:19-04	2026-08-12 19:24:44.394187-04	cerrada_sin_cuadre	50471f55-7b80-41ea-86ee-4e91338d3864	\N	\N	t	Reconstruida por la migración 016	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	2026-08-12 19:24:44.394187-04
\.
...
COPY public.pago_prestamo (id, prestamo_id, cuota_id, registrado_por, monto_total, aplicado_mora, aplicado_interes, aplicado_capital, tipo, metodo_pago, referencia, nota, fecha, created_at) FROM stdin;
\.
...
f0279557-83f8-42df-927b-8d70a9e31ce2	2026-08-13	10000.0000	\N	10:38:37	50471f55-7b80-41ea-86ee-4e91338d3864	2026-08-13 10:38:38.623337-04	9500.0000	\N	f	\N	2026-08-13 10:38:38.623337-04
\.
...
COPY public.prestamo (id, proveedor_id, registrado_por, monto_principal, tasa_interes_pct, tipo_interes, modalidad_pago, tipo_plazo, plazo_meses, num_cuotas, fecha_desembolso, fecha_vencimiento, saldo_capital, interes_acumulado, mora_acumulada, total_pagado, estado, nota, created_at) FROM stdin;
3c6957f8-385c-47ea-b609-163c6a1fc4a4	b249aa85-eb6c-4875-a8ab-2b56bff9c420	50471f55-7b80-41ea-86ee-4e91338d3864	30000.00	0.00	exento	vencimiento	cosecha_mayor	\N	\N	2026-08-13	2027-03-31	30000.00	0.00	0.00	0.00	activo	\N	2026-08-13 17:57:54.905291-04
8ad67967-6973-4d8c-a836-12c15f317156	965aed56-a93d-4e2f-8a24-bc1d90c6f27d	50471f55-7b80-41ea-86ee-4e91338d3864	20000.00	1.50	simple	vencimiento	cosecha_mayor	\N	\N	2026-08-13	2027-03-31	20000.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:04:18.864784-04
aee5b04f-bf72-4d1f-8bcc-f185baf28049	b6426798-4857-4045-afa2-9c54d4664d26	50471f55-7b80-41ea-86ee-4e91338d3864	15000.00	1.50	simple	vencimiento	cosecha_mayor	\N	\N	2026-08-13	2027-03-31	15000.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:06:02.102096-04
a377f48a-91d3-4ecd-b566-5f8af2f2ebe1	a52f733a-1268-4cb4-86e9-d2ae944d2008	50471f55-7b80-41ea-86ee-4e91338d3864	93600.00	1.50	simple	vencimiento	cosecha_mayor	\N	\N	2026-08-13	2027-03-31	93600.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:08:59.332229-04
a5cf635a-6d0e-43cb-a071-3cbf07a69e4e	41b28d96-087d-4481-ad9c-f212665a3009	50471f55-7b80-41ea-86ee-4e91338d3864	15000.00	1.50	simple	vencimiento	cosecha_mayor	\N	\N	2026-08-13	2027-03-31	15000.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:11:39.865306-04
bc360da9-d36c-4eb2-b0bc-8b047bf5c37d	96abd074-8801-4d63-95d3-f32b6d739523	50471f55-7b80-41ea-86ee-4e91338d3864	87300.00	1.50	simple	abonos_libres	meses	\N	\N	2026-08-13	\N	87300.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:21:23.23128-04
adf3e377-fee1-4ccd-83fb-24b72a54d107	8e1e86bc-aef7-4a09-a707-6c7035e8434a	50471f55-7b80-41ea-86ee-4e91338d3864	29000.00	0.00	exento	abonos_libres	meses	\N	\N	2026-08-13	\N	29000.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:24:42.10616-04
611fd4d1-e1bc-490c-afdb-90c1ce13749e	8e1e86bc-aef7-4a09-a707-6c7035e8434a	50471f55-7b80-41ea-86ee-4e91338d3864	1000.00	0.00	exento	abonos_libres	meses	\N	\N	2026-08-13	\N	1000.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:25:46.043662-04
a0f0ed56-2fad-4ac5-acf1-01e14d987dec	0abed074-d391-433f-9588-11ffe34a2644	50471f55-7b80-41ea-86ee-4e91338d3864	60000.00	0.00	exento	abonos_libres	meses	\N	\N	2026-08-13	\N	60000.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:27:26.229741-04
9d01542f-ff9b-4634-8d7a-703078a223d5	b6426798-4857-4045-afa2-9c54d4664d26	50471f55-7b80-41ea-86ee-4e91338d3864	5000.00	1.50	simple	vencimiento	cosecha_mayor	\N	\N	2026-08-13	2027-03-31	5000.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:28:00.139396-04
1ba58938-3549-469c-aa74-c0d9d024edf6	19245b2a-6294-48a2-a65a-3650a9bfa1a4	50471f55-7b80-41ea-86ee-4e91338d3864	122400.00	1.50	simple	abonos_libres	meses	\N	\N	2026-08-13	\N	122400.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:36:07.047201-04
03a2c264-9653-44b0-9e9b-90209e8ccf29	3eb2e7d6-59e3-485e-9a1b-98c5f6a5d434	50471f55-7b80-41ea-86ee-4e91338d3864	19100.00	1.50	simple	abonos_libres	meses	\N	\N	2026-08-13	\N	19100.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:37:03.416005-04
852ecd09-60d7-4dcd-afa3-d482c61d0ad4	597f4e2d-78b2-45f4-86c7-384b64031a16	50471f55-7b80-41ea-86ee-4e91338d3864	10000.00	1.50	simple	abonos_libres	meses	\N	\N	2026-08-13	\N	10000.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:37:46.990106-04
ee5680f7-fedc-4aff-bae7-0721e928face	063b5ad5-340c-447e-8999-9e1f461e1982	50471f55-7b80-41ea-86ee-4e91338d3864	8800.00	0.00	exento	abonos_libres	meses	\N	\N	2026-08-13	\N	8800.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:38:35.204708-04
d1596b9b-0798-4218-a462-3105c5dd602e	15df5fed-5685-4e21-a633-a430a3ef671d	50471f55-7b80-41ea-86ee-4e91338d3864	18000.00	1.50	simple	abonos_libres	meses	\N	\N	2026-08-13	\N	18000.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:39:19.997708-04
4c920795-97ee-478e-b237-a2be4343248f	e7753806-8abf-4f62-a440-f8b8f9d92e49	50471f55-7b80-41ea-86ee-4e91338d3864	6500.00	0.00	exento	abonos_libres	meses	\N	\N	2026-08-13	\N	6500.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:40:47.431712-04
3192029f-b9f1-4225-87e6-9cc118b294a9	d1cacdbd-c5c1-4646-ac26-2bd42f168f4c	50471f55-7b80-41ea-86ee-4e91338d3864	79500.00	1.50	simple	abonos_libres	meses	\N	\N	2026-08-13	\N	79500.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:41:35.076108-04
00f0f10a-4dd9-4d66-aa98-2ec5d6cfc9f6	aae92984-5596-4cf5-bab3-a2edf97f4677	50471f55-7b80-41ea-86ee-4e91338d3864	160000.00	0.00	exento	abonos_libres	meses	\N	\N	2026-08-13	\N	160000.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:53:18.573889-04
0e21a6e7-9c5c-48a6-95ca-f7d779824b0f	4bd35246-1332-49db-89d2-db13ae054ca7	50471f55-7b80-41ea-86ee-4e91338d3864	38000.00	0.00	exento	abonos_libres	meses	\N	\N	2026-08-13	\N	38000.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:53:49.25707-04
c183193e-6ed8-4652-bc7b-96e103b68a25	360543ee-0192-48b7-aa87-5eb897047a3b	50471f55-7b80-41ea-86ee-4e91338d3864	100000.00	1.50	simple	abonos_libres	meses	\N	\N	2026-08-13	\N	100000.00	0.00	0.00	0.00	activo	\N	2026-08-13 18:55:11.119011-04
\.
...
c262506f-26a3-495d-9c59-556f5f4bab58	HECTOR BIENVENIDO ORTEGA	\N	8293596934	ocasional	0.00	t	2026-08-11 22:56:04.031082-04	t
...
SELECT pg_catalog.setval('public.jornada_numero_seq', 7, true);
...
ALTER TABLE ONLY public.finca_proveedor
    ADD CONSTRAINT finca_proveedor_proveedor_id_fkey FOREIGN KEY (proveedor_id) REFERENCES public.proveedor(id);
...
--
-- Name: FUNCTION registrar_abono(p_compra_id uuid, p_monto numeric, p_metodo text, p_registrado_por uuid); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.registrar_abono(p_compra_id uuid, p_monto numeric, p_metodo text, p_registrado_por uuid) TO anon;
GRANT ALL ON FUNCTION public.registrar_abono(p_compra_id uuid, p_monto numeric, p_metodo text, p_registrado_por uuid) TO authenticated;
GRANT ALL ON FUNCTION public.registrar_abono(p_compra_id uuid, p_monto numeric, p_metodo text, p_registrado_por uuid) TO service_role;


--
-- Name: FUNCTION registrar_pago_prestamo(p_prestamo_id uuid, p_monto numeric, p_metodo text, p_registrado_por uuid, p_nota text, p_referencia text); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.registrar_pago_prestamo(p_prestamo_id uuid, p_monto numeric, p_metodo text, p_registrado_por uuid, p_nota text, p_referencia text) TO anon;
GRANT ALL ON FUNCTION public.registrar_pago_prestamo(p_prestamo_id uuid, p_monto numeric, p_metodo text, p_registrado_por uuid, p_nota text, p_referencia text) TO authenticated;
GRANT ALL ON FUNCTION public.registrar_pago_prestamo(p_prestamo_id uuid, p_monto numeric, p_metodo text, p_registrado_por uuid, p_nota text, p_referencia text) TO service_role;
...
\unrestrict 81Vy9ufiwHMB7HEEuPnadekCltvp5a2gXwRQmtI9FUJHoed4aYAw1NsIbuuaqdS
